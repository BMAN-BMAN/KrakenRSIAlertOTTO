...
if questionCheck.lower() in ["yes", "yes.", "y"]:
    ...
    if codeCheck == 2121:
        passCheck = True
        print("Very well. Let us begin monitoring for automatic buy-in opportunities for your chosen coin.")
    else:
...
# Continuously fetch and print RSI
pair = "XXBTZUSD"
interval_seconds = 60  # Adjust as needed
while passCheck:
    # Getting hourly ticker data for BTC/USD
    ticker_data = get_kraken_hourly_ticker(pair, interval=interval_seconds)

    # Calculate RSI based on hourly data
    rsi = calculate_rsi(ticker_data, period=14)
